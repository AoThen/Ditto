...
OCR_API void* OcrInit(const char* modelsDir)
{
    try {
        ...
        auto detSession = std::make_shared<Ort::Session>(*env, detPath.c_str(), sessionOptions);
        auto recSession = std::make_shared<Ort::Session>(*env, recPath.c_str(), sessionOptions);
